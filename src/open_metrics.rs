...
use openmetrics_parser::OpenMetricsValue;
use std::error::Error;
use std::ffi::CString;
...
#[repr(C)]
pub struct OpenMetricsParams {
    pub url: String,
    pub source: Option<String>,
}

#[repr(C)]
pub struct OpenMetricsInitData {
    pub done: AtomicBool,
}

pub struct OpenMetricsVTab;

impl VTab for OpenMetricsVTab {
    type InitData = OpenMetricsInitData;
    type BindData = OpenMetricsParams;
    ...
    fn func(
        ...
        if init_data.done.swap(true, Ordering::Relaxed) {
            ...
            let vector_detail = output.flat_vector(6);
            let mut vector_value = output.flat_vector(2);
            let slice_value = vector_value.as_mut_slice::<f64>();
            let vector_labels = output.flat_vector(3);
            let vector_source = output.flat_vector(4);
            let mut vector_created_at = output.flat_vector(5);
            let slice_created_at = vector_created_at.as_mut_slice::<u64>();
            if let Ok(mut response) = ureq::get(http_url)
                .config()
                ...
                .call()
            {
                if response.status().is_success() {
                    if let Ok(body) = response.body_mut().read_to_string() {
                        if let Ok(exposition) = parse_openmetrics(&body) {
                            ...
                            for (metric_name, family) in &exposition.families {
                                ...
                                for sample in family.iter_samples() {
                                    ...
                                    vector_source
                                        .insert(sample_index, CString::new(source.as_str())?);
                                    ...
                                    match &sample.value {
                                        OpenMetricsValue::Unknown(value) => {
                                            slice_value[sample_index] = value.as_f64();
                                        }
                                        OpenMetricsValue::Untyped(value) => {
                                            slice_value[sample_index] = value.as_f64();
                                        }
                                        OpenMetricsValue::Gauge(gauge) => {
                                            slice_value[sample_index] = gauge.as_f64();
                                        }
                                        OpenMetricsValue::Counter(counter) => {
                                            slice_value[sample_index] = counter.value.as_f64();
                                        }
                                        OpenMetricsValue::Info => {
                                            slice_value[sample_index] = 0.0;
                                        }
                                        OpenMetricsValue::StateSet(state_set) => {
                                            slice_value[sample_index] = state_set.as_f64();
                                        }
                                        ...
                                        | OpenMetricsValue::GaugeHistogram(histogram) => {
                                            ...
                                            vector_detail
                                                .insert(sample_index, CString::new(details)?);
                                        }
                                        OpenMetricsValue::Summary(summary) => {
                            ...
                            return Err(Box::new(std::io::Error::new(
                                ...
                                "Failed to parse OpenMetrics text",
                            )));
                        }
                    }
                } else {
                ...
                return Err(Box::new(std::io::Error::new(
                    std::io::ErrorKind::Other,
                    format!("Failed to connect URL: {}", http_url),
                )));
            }
        }
        Ok(())
